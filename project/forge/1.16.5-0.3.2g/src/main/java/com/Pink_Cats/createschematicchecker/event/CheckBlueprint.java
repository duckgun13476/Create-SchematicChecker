...
public class CheckBlueprint {
    ...
    private void applyResultOnServerThread(ServerPlayerEntity player, String id, boolean pass) {
        ...
        RegistryKey<World> dimension = meta.dim;
    ...
    private void sendNoticeToOperator(ServerPlayerEntity player, String notice) {
        runOnServerMainThread(player, () -> {
            ...
            for (int i = 0; i < lines.length; i++) {
                ...
                player.sendMessage(
                        new StringTextComponent(lines[i]).setStyle(Style.EMPTY.applyFormatting(color)),
                        Util.DUMMY_UUID
                );
            }
        });
    ...
    public static void broadcast(String playerBlueprintId, String playerName) {
        for (final ServerPlayerEntity player : getAllOnlinePlayers()) {
            player.sendMessage(
                    ...
                    ).setStyle(Style.EMPTY.applyFormatting(TextFormatting.GOLD)),
                    Util.DUMMY_UUID
            );
        }
    }
