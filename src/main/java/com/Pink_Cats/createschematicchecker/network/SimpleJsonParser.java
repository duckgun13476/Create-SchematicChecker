...
public class SimpleJsonParser {
     ...
     *         若解析失败，返回null
     */
    public static String[][][] parseJsonToArrays(String filePath) {
        try {
            ...
            if (!validateBasicFormat(lines)) {
                return null;
            }

            // 关键修改：字段缺失时返回空数组，而非null
            String[][] idArray = parseTwoDimensionalArray(lines, "id");
            String[][] operateArray = parseTwoDimensionalArray(lines, "operate");
            ...
            e.printStackTrace(); // 增加堆栈打印，便于定位异常
            return null;
        }
    }
    ...
    // 基础格式校验
    private static boolean validateBasicFormat(List<String> lines) {
        if (lines == null || lines.isEmpty()) {
            System.err.println("empty JSON file");
            return false;
        }
        if (!lines.get(0).startsWith("{")) {
            System.err.println("JSON应以'{'开头（行号：1） JSON should start with{");
            return false;
        }
        if (!lines.get(lines.size() - 1).endsWith("}")) {
            System.err.println("JSON应以'}'结尾（行号： Json should end with }" + lines.size() + "）");
            return false;
        }
        return true;
    }
    ...
    // 解析指定字段的二维数组
    private static String[][] parseTwoDimensionalArray(List<String> lines, String fieldName) {
        // 1. 找到目标字段所在行
        ...
        if (arrayStart == -1) {
            System.err.println(fieldName + "字段格式错误，缺少'['（行号： String miss [" + (fieldLineIndex + 1) + "）");
            return null;
        }
        ...
        // 4. 解析二维数组
        return parseTwoDimensionalArrayString(arrayContent.toString());
    }
    ...
    // 解析二维数组字符串为String[][]
    private static String[][] parseTwoDimensionalArrayString(String arrayStr) {
        // 去除最外层的[]
        ...
        for (String subArrayStr : subArrayStrs) {
            String[] subArray = parseOneDimensionalArrayString(subArrayStr);
            if (subArray == null) {
                return null;
            }
    ...
    // 解析一维数组字符串为String[]
    private static String[] parseOneDimensionalArrayString(String arrayStr) {
        if (!arrayStr.startsWith("[") || !arrayStr.endsWith("]")) {
            System.err.println("miner format wrong: " + arrayStr);
            return null;
        }
    ...
    private static boolean ConfigIsValid(String[][][] result) {
        if (result != null) {
            ...
            if (idArray.length != 0) {
                ...
                for (String[] subArray : idArray) {
                    if (subArray.length != 2) {
                        Message.FM("Length wrong: " + subArray.length);
                        return false;
                    }

                    if (!subArray[0].contains(":")) {
                        Message.FM("id wrong: " + subArray[0]);
                        return false;
                    }
            ...
            if (operateArray.length != 0) {
                ...
                for (String[] subArray : operateArray) {
                    ...
                    if (!subArray[2].contains("operate") ) {
                        Message.FM("Miss operate: " + subArray[2]);
                        return false;
                    }
                    if (!subArray[2].contains("limit") && !subArray[2].contains("clear") && !subArray[2].contains("replace") ) {
                        Message.FM("Miss operate way [limit/clear/replace]: " + subArray[2]);
                        return false;
                    }
                }
            }
     ...
     * @return 操作是否成功
     */
    public static boolean createUserRuleFile(String directoryPath, String fileName) {
        // 构建完整文件路径
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            System.err.println("创建文件或写入内容失败:  makefile or write fail " + e.getMessage());
            return false;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        String jsonPath = "CSC/online/rule_2025_10_01_09_35_32.json";
        String UserRulePath = "CSC";
        ...
        try {
            ...
            String[][][] result = parseJsonToArrays(jsonPath);
            boolean valid = ConfigIsValid(result);
            if (valid) {
                if (result != null) {
                    idArray = result[0];
                }
            ...
            Message.FM("load online rule fail");
            e.printStackTrace();
        }
        ...
        try {
            ...
            String[][][] result2 = parseJsonToArrays(UserRulePath +"/"+ UserRuleFile);
            boolean valid2 = ConfigIsValid(result2);
            if (valid2) {
                if (result2 != null) {
                    idArray2 = result2[0];
                }
            ...
            Message.FM("load user rule fail");
            e.printStackTrace();
        }
