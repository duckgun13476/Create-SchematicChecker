...
public class NbtFunc {
    ...
    public Map<String, Object> NBTCheck(CompoundTag nbt_data,List<String> CheatLog)//鏍￠獙 NBT
    {
        ...
        try {
            ...
            TripwireScanResult tripwireScanResult = scanAndSanitizeTripwireStates(palette, blocks);
    ...
    private static TripwireScanResult scanAndSanitizeTripwireStates(ListTag palette, ListTag blocks) {
        ...
        for (int i = 0; i < blocks.size(); i++) {
            ...
            if ("minecraft:tripwire".equals(id)) {
                TripwireBlockState tripwire = new TripwireBlockState(pos, id, properties);
                tripwireBlocks.put(posKey, tripwire);
            } else if ("minecraft:tripwire_hook".equals(id)) {
                hooks.add(new TripwireBlockState(pos, id, properties));
            } else if (isTripwireWaterUpdateSource(id, properties)) {
                waterUpdateSources.add(posKey);
            } else if (isTripwireRedstoneUpdateSource(id, properties)) {
        ...
        int sanitizedPaletteCount = sanitizeTripwirePaletteStates(palette);

        TripwireScanResult result = new TripwireScanResult();
        result.cheat = cheat;
        result.sanitizedPaletteCount = sanitizedPaletteCount;
        result.hookCount = hooks.size();
        result.tripwireCount = tripwireBlocks.size();
        result.hasWaterUpdateSource = !waterUpdateSources.isEmpty();
        result.hasRedstoneUpdateSource = !redstoneUpdateSources.isEmpty();
        return result;
    }

    private static int sanitizeTripwirePaletteStates(ListTag palette) {
        int changedCount = 0;
        for (int i = 0; i < palette.size(); i++) {
            CompoundTag paletteItem = palette.getCompound(i);
            String id = PaletteGetId(paletteItem);
    ...
    private static boolean isTripwireWaterUpdateSource(String id, CompoundTag properties) {
    ...
    private static boolean isTripwireRedstoneUpdateSource(String id, CompoundTag properties) {
    ...
    private static class TripwireScanResult {
        boolean cheat;
    ...
    private static class TripwireBlockState {
        final int[] pos;
        final String id;
        final CompoundTag properties;

        TripwireBlockState(int[] pos, String id, CompoundTag properties) {
            this.pos = pos;
            this.id = id;
            this.properties = properties;
        }

        boolean isPoweredOrAttached() {
            return "true".equals(propertyValue(properties, "powered"))
                    || "true".equals(propertyValue(properties, "attached"));
        }

        boolean isPoweredOrAttachedOrDisarmed() {
